...
- Supabase live : projet `ucymjsxmnzdxvvupgaof` — migrations 0001→0023 + `places_menu_urls` (0030) appliquées, Edge Functions ACTIVE : `otp-send` v3 + `otp-verify` v3 (⚠️ verify_jwt=true depuis le redeploy MCP du 07/07 — OK, l'app envoie Bearer anon), `moderate-spawter`, `seed-inventory`. **OTP : mock sur OPT-IN EXPLICITE** (`MOCK_TERMII=true`, sécurité C1 — un prod sans MOCK_TERMII=true ni `TERMII_API_KEY` échoue fermé `edge_misconfigured` au lieu d'accepter `123456` pour tout numéro) : code de test `123456` (6 chiffres, aligné pin Termii). Prod SMS réel : poser `TERMII_API_KEY` (et ne PAS poser MOCK_TERMII). Staging/CI : `MOCK_TERMII=true`. Un backend self-hosted (Coolify VPS) tourne en parallèle avec les mêmes fonctions (cutover envisagé, cf. sprint-status).
